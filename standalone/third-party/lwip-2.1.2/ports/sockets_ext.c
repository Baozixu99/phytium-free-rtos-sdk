...
#define LWIP_SOCKOPT_CHECK_OPTLEN_CONN_PCB(sock, optlen, opttype)     \
    do                                                                \
    {                                                                 \
        if (((optlen) < sizeof(opttype)) || ((sock)->conn == NULL) || \
    ...
    switch (optname)
    {
        case IPV6_MULTICAST_IF: /* NB: like IP_MULTICAST_IF, this returns an IP not an index */
            ...
            *(u8_t *)optval = udp_get_multicast_netif_index(sock->conn->pcb.udp);
            LWIP_DEBUGF(SOCKETS_DEBUG, ("lwip_getsockopt_ext(IPPROTO_IPV6, "
                                        "IPV6_MULTICAST_IF) = 0x%" X32_F "\n",
                                        *(u32_t *)optval));
            break;
        case IPV6_MULTICAST_HOPS:
            LWIP_DEBUGF(SOCKETS_DEBUG, ("IPV6_MULTICAST_HOPS\n"));
            LWIP_SOCKOPT_CHECK_OPTLEN_CONN_PCB(sock, *optlen, u8_t);
            if (NETCONNTYPE_GROUP(netconn_type(sock->conn)) != NETCONN_UDP)
            {
                *err = ENOPROTOOPT;
            ...
            *(u8_t *)optval = udp_get_multicast_ttl(sock->conn->pcb.udp);
            LWIP_DEBUGF(SOCKETS_DEBUG,
                        ("lwip_getsockopt_ext(IPPROTO_IPV6, IP_MULTICAST_LOOP) = %d\n",
                         *(int *)optval));
            break;
        case IPV6_MULTICAST_LOOP:
            LWIP_SOCKOPT_CHECK_OPTLEN_CONN_PCB(sock, *optlen, u8_t);
