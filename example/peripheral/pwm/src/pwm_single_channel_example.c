 ...
 *
 *
 * FilePath: pwm_single_channel_example.c
...
#define FPWM_DEBUG_TAG "PWM_SINGLE_CHANNEL_TEST"
...
#define PWM_SINGLE_CHANNEL_TEST_TASK_PRIORITY 3
...
static FError PwmInit(u32 pwm_id)
{
    ...
    /* init iomux */
    FIOMuxInit();
    FIOPadSetPwmMux(pwm_id, PWM_TEST_CHANNEL);
...
static void FFreeRTOSSingleChannelTask()
{
    FError ret = FPWM_SUCCESS;
...
exit:
    ...
    {
        printf("%s@%d: pwm single channel example [failure], task_res = %d\r\n", __func__, __LINE__, task_res);
        return pdFAIL;
    }
    else
    {
        printf("%s@%d: pwm single channel example [success].\r\n", __func__, __LINE__);
        return pdTRUE;
    }
}
