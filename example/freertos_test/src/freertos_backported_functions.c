...
void QueueFuncTest(void)
{
    ...
    FASSERT(pdTRUE==xQueueSendToBack(handle, &queue_item_to_send, DELAY_TICKS));
    vTaskDelay(1);
    ...
    for(i = 0; i < ITEM_SIZE; i++)
    {
        ...
        FASSERT(queue_item_to_send[i]==queue_item_received[i]);  
    }
    ...
    FASSERT(pdTRUE==xSemaphoreGive(handle));
    vTaskDelay(1);

    FASSERT(pdTRUE==xSemaphoreTake(handle, DELAY_TICKS));
    vTaskDelay(1);
    ...
    FASSERT(NO_OF_ITEMS== uxSemaphoreGetCount(handle));    
    for(i = 0; i < NO_OF_ITEMS; i++)
    {
        FASSERT(pdTRUE==xSemaphoreTake(handle, DELAY_TICKS));
    }
    vTaskDelay(1);
    FASSERT(0==uxSemaphoreGetCount(handle));
    vSemaphoreDelete(handle);
    ...
    FASSERT(((void *)xTaskGetCurrentTaskHandle()) == xSemaphoreGetMutexHolder(handle)); 
    FASSERT(pdTRUE==xSemaphoreGive(handle));
    vTaskDelay(1);
     /*Mutex should have been released*/
    FASSERT(NULL==xSemaphoreGetMutexHolder(handle));     
    vSemaphoreDelete(handle);
    ...
    for(i = 0; i < NO_OF_ITEMS; i++)
    {
        FASSERT(pdTRUE==xSemaphoreTakeRecursive(handle, DELAY_TICKS));
    }
    vTaskDelay(1);
    ...
    for(i = 0; i < NO_OF_ITEMS; i++)
    {
        FASSERT(pdTRUE==xSemaphoreGiveRecursive(handle));
    }
    vTaskDelay(1);
    /*Mutex should have been released*/
    FASSERT(NULL==xSemaphoreGetMutexHolder(handle));      
...
void StaticTaskCreateTest(void)
{
    ...
    FASSERT(NULL!= handle);    /*Check static task was successfully allocated*/
    FASSERT(has_run);    /*Check static task has run*/
...
void StaticEventGroupTest(void)
{
    ...
    FASSERT(WAIT_BITS==xEventGroupWaitBits(eg_handle, WAIT_BITS, pdTRUE, pdTRUE, portMAX_DELAY));
