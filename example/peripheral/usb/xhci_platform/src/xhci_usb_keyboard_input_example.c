...
enum
{
    XHCI_USB_KEYBOARD_SUCCESS = 0,
    XHCI_USB_KEYBOARD_FAIL    = 1,
};
...
task_exit:
    ret = (ret == 0) ? XHCI_USB_KEYBOARD_SUCCESS : XHCI_USB_KEYBOARD_FAIL;
...
BaseType_t FFreeRTOSRunXhciKeyboard(u32 id, const char *devname)
{
    ...
    int task_res = XHCI_USB_KEYBOARD_SUCCESS;
...
exit:
    ...
    if (task_res != XHCI_USB_KEYBOARD_SUCCESS)
    {
