...
#define nvme_mmio_read_4(b_a, reg)					\
	FtIn32((uintptr)b_a +  nvme_mmio_offsetof(reg))

#define nvme_mmio_write_4(b_a, reg, val)				\
	FtOut32((uintptr)b_a+nvme_mmio_offsetof(reg),val)


#define nvme_mmio_write_8(b_a, reg, val)	\
	do {								\
		FtOut32((uintptr)b_a+nvme_mmio_offsetof(reg),val & 0xFFFFFFFF);	\
		FtOut32((uintptr)b_a+nvme_mmio_offsetof(reg)+4,(val & 0xFFFFFFFF00000000ULL) >> 32); \
	} while (0)
