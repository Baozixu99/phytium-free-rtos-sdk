...
sdmmc_err_t sdmmc_init_mmc_read_ext_csd(sdmmc_card_t *card)
{
    ...
    uint8_t *ext_csd = sdmmc_port_align_malloc(EXT_CSD_MMC_SIZE, TRUE);
    if (!ext_csd)
    {
    ...
    card->max_freq_khz = min((int)card->max_freq_khz, card->host.max_freq_khz);
...
sdmmc_err_t sdmmc_init_mmc_bus_width(sdmmc_card_t *card)
{
    ...
    if (card->ext_csd.power_class != 0)
    {
        ...
        if (err != SDMMC_OK)
        {
            SDMMC_LOGE(TAG, "%s: can't change power class (%d bit), 0x%x", __func__, card->ext_csd.power_class, err);
            return err;
        }
    }
