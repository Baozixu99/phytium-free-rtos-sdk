		    ...
		    void **img_info)
{
	...
	while (1) {
		...
		if (da != RPROC_LOAD_ANYADDR) {
			...
			pa = METAL_BAD_PHYS;
			size_t temp_nmemsize;
			if(nmemsize % 0x1000)
			{
				// ��temp_nmemsize����4K ���϶���
				temp_nmemsize = (nmemsize / 0x1000 + 1) * 0x1000;
				printf("load: nmemsize 0x%lx, temp_nmemsize 0x%lx\r\n", nmemsize, temp_nmemsize);
				nmemsize = temp_nmemsize;
			}
	...
	if (rsc_table) {
		...
		size_t temp_rsc_size;
		if(temp_rsc_size % 0x1000)
		{
			// ��temp_rsc_size����4K ���϶���
			temp_rsc_size = (rsc_size / 0x1000 + 1) * 0x1000;
			printf("load: rsc_size 0x%lx, temp_rsc_size 0x%lx\r\n", rsc_size, temp_rsc_size);
			rsc_size = temp_rsc_size;
		}
