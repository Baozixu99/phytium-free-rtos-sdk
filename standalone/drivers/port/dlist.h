...
#ifndef COMMON_DIST_H
#define COMMON_DIST_H
 ...
 *	int *ptr = &my_foo.bar;
 *
 *	struct foo *container = DIST_CONTAINER_OF(ptr, struct foo, bar);
 *
 ...
 */
#define DIST_CONTAINER_OF(ptr, type, field) \
...
#define SYS_DLIST_FOR_EACH_NODE_SAFE(__dl, __dn, __dns)                                     \
    for ((__dn) = sys_dlist_peek_head(__dl), (__dns) = sys_dlist_peek_next((__dl), (__dn)); \
         (__dn) != NULL; (__dn) = (__dns), (__dns) = sys_dlist_peek_next(__dl, __dn))
...
#define SYS_DLIST_CONTAINER(__dn, __cn, __n) \
    (((__dn) != NULL) ? DIST_CONTAINER_OF(__dn, __typeof__(*(__cn)), __n) : NULL)
/**
...
#define SYS_DLIST_PEEK_NEXT_CONTAINER(__dl, __cn, __n) \
    (((__cn) != NULL) ? SYS_DLIST_CONTAINER(sys_dlist_peek_next((__dl), &((__cn)->__n)), __cn, __n) : NULL)
...
#define SYS_DLIST_FOR_EACH_CONTAINER_SAFE(__dl, __cn, __cns, __n) \
    for ((__cn) = SYS_DLIST_PEEK_HEAD_CONTAINER(__dl, __cn, __n), \
        (__cns) = SYS_DLIST_PEEK_NEXT_CONTAINER(__dl, __cn, __n); \
         (__cn) != NULL;                                          \
         (__cn) = (__cns), (__cns) = SYS_DLIST_PEEK_NEXT_CONTAINER(__dl, __cn, __n))
