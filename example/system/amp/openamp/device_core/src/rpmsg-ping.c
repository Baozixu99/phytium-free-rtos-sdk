...
#define     PING_DEV_DRIVER_DEBUG_TAG "    DRIVER_01"
#define     PING_DEV_DRIVER_DEBUG_I(format, ...) FT_DEBUG_PRINT_I( PING_DEV_DRIVER_DEBUG_TAG, format, ##__VA_ARGS__)
#define     PING_DEV_DRIVER_DEBUG_W(format, ...) FT_DEBUG_PRINT_W( PING_DEV_DRIVER_DEBUG_TAG, format, ##__VA_ARGS__)
#define     PING_DEV_DRIVER_DEBUG_E(format, ...) FT_DEBUG_PRINT_E( PING_DEV_DRIVER_DEBUG_TAG, format, ##__VA_ARGS__)
/**************************** Type Definitions *******************************/
                             ...
                             uint32_t src, void *priv)
{
    ...
    for (i = 0; i < (int)r_payload->size; i++)
    {
        ...
        {
            PING_DEV_DRIVER_DEBUG_I("Data corruption at index %d.\r\n", i);
            PING_DEV_DRIVER_DEBUG_I("Want data is %d.\r\n", flg_cnt);
            PING_DEV_DRIVER_DEBUG_I("Get data is %d.\r\n", r_payload->data[i]);
            err_cnt++;
            break;
        }
    }
                                       ...
                                       const char *name, uint32_t dest)
{
    ...
    {
        PING_DEV_DRIVER_DEBUG_E("Unexpected name service %s.\r\n", name);
    }
    else
...
static int app(struct rpmsg_device *rdev, void *priv)
{
    ...
    PING_DEV_DRIVER_DEBUG_I("Send data to remote core, retrieve the echo.");
    PING_DEV_DRIVER_DEBUG_I(" and validate its integrity ..\r\n");
...
int rpmsg_ping(struct rpmsg_device *rdev, void *priv)
{
    ...
    if (ret != 0)
    {
        PING_DEV_DRIVER_DEBUG_E("Rpmsg_ping application error,code:0x%x",ret);
        return ret;
    }
