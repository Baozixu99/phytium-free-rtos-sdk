...
{
    assert(para);
    _UNUSED fsdmmchost_dev_t *dev = (fsdmmchost_dev_t *)para;
                                         ...
                                         FSdmmcCmd *out_cmd, FSdmmcData *out_data)
{
    ...
    if ((kSD_ApplicationSendScr == cmd_ind)  || (kSD_ApplicationStatus == cmd_ind))
    {
        out_cmd->flag |= FSDMMC_CMD_FLAG_ADTC;
    }
    ...
    if (NULL != in_trans->data)
    {
        ...
        if (in_trans->data->rxData)
        {
            ...
            out_data->buf_dma = (uintptr)in_trans->data->rxData;
        }
        else
        {
            out_cmd->flag |= FSDMMC_CMD_FLAG_WRITE_DATA;
            out_data->buf = (uint8_t *)in_trans->data->txData;
            out_data->buf_dma = (uintptr)in_trans->data->txData;
        }
