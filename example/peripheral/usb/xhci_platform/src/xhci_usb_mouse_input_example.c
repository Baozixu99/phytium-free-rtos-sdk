...
static void XhciUsbMouseTask(void *args)
{
    int ret;
    u8 id = (u8)usb_id;
    ...
    vTaskDelay(XHCI_INPUT_WAIT_TIME); /* wait user input for a while */

    printf("Input finished, exit...\r\n");

task_exit:
    ret = (ret == 0) ? XHCI_USB_MOUSE_SUCCESS : XHCI_USB_MOUSE_FAIL;
...
BaseType_t FFreeRTOSRunXhciMouse(u32 id, const char *devname)
{
    ...
    int task_res = XHCI_USB_MOUSE_SUCCESS;
    ...
    ret = xTaskCreate((TaskFunction_t)XhciUsbMouseTask,
                      (const char *)"XhciUsbMouseTask",
                      (uint16_t)4096,
                      (void *)devname,
...
exit:
    ...
    if (task_res != XHCI_USB_MOUSE_SUCCESS)
    {
        printf("%s@%d: XHCI usb mouse example [failure], task_res = %d\r\n", __func__, __LINE__, task_res);
        return pdFAIL;
    }
    else
    {
        printf("%s@%d: XHCI usb mouse example [success].\r\n", __func__, __LINE__);
        return pdTRUE;
    }
}