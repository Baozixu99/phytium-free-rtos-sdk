...
typedef void (*nvme_poll_completion_cb)(void *arg, const struct nvme_completion *cpl) ;
...
int nvme_ctrlr_cmd_identify_controller(struct nvme_controller *ctrlr,
				       nvme_poll_completion_cb cb_fn, void *cb_arg);
...
static inline
struct nvme_request *nvme_allocate_request(struct nvme_controller *ctrlr,nvme_poll_completion_cb cb_fn, void *cb_arg)
{
	struct nvme_request *request;

	request = nvme_cmd_request_alloc(ctrlr);
	if (request != NULL) {
		request->cb_fn = cb_fn;
...
static inline
struct nvme_request *nvme_allocate_request_vaddr(struct nvme_controller *ctrlr,void *payload,
						 uint32_t payload_size,
						 nvme_poll_completion_cb cb_fn,
						 void *cb_arg)
{
	...
	request = nvme_allocate_request(ctrlr,cb_fn, cb_arg);
	if (request != NULL) {
...
static inline
struct nvme_request *nvme_allocate_request_null(struct nvme_controller *ctrlr,nvme_poll_completion_cb cb_fn,
						void *cb_arg)
{
	struct nvme_request *request;
