...
#define     VIRTIO_DEV_DRIVER_DEBUG_TAG "    DRIVER_02"
#define     VIRTIO_DEV_DRIVER_DEBUG_I(format, ...) FT_DEBUG_PRINT_I( VIRTIO_DEV_DRIVER_DEBUG_TAG, format, ##__VA_ARGS__)
#define     VIRTIO_DEV_DRIVER_DEBUG_W(format, ...) FT_DEBUG_PRINT_W( VIRTIO_DEV_DRIVER_DEBUG_TAG, format, ##__VA_ARGS__)
#define     VIRTIO_DEV_DRIVER_DEBUG_E(format, ...) FT_DEBUG_PRINT_E( VIRTIO_DEV_DRIVER_DEBUG_TAG, format, ##__VA_ARGS__)
			     ...
			     uint32_t src, void *priv)
{
	...
	VIRTIO_DEV_DRIVER_DEBUG_I(" seed %s: \r\n", seed);
	...
	rnum++;
	VIRTIO_DEV_DRIVER_DEBUG_I(" rnum %d: \r\n", rnum);
	if (rnum == MSG_LIMIT) 
	{
		ept_deleted = 1;
	}
				       ...
				       const char *name, uint32_t dest)
{
	...
	if (strcmp(name, RPMSG_SERVICE_NAME))
		VIRTIO_DEV_DRIVER_DEBUG_E("Unexpected name service %s.\r\n", name);
	else
...
static int app(struct rpmsg_device *rdev, void *priv)
{
	...
	VIRTIO_DEV_DRIVER_DEBUG_I(" 1 - Send data to remote core, retrieve the echo");
	...
	for (i = 1; i <= MSG_LIMIT; i++) {
		...
		VIRTIO_DEV_DRIVER_DEBUG_I("rpmsg sample test: message %d sent\r\n", i);
...
int rpmsg_sample_ping(struct rpmsg_device *rdev, void *priv)
{
	...
	if (ret != 0)
    {
        VIRTIO_DEV_DRIVER_DEBUG_E("Rpmsg_sample_ping application error,code:0x%x",ret);
        return ret;
    }
