...
#define CIS_TUPLE(NAME) \
    (cis_tuple_t) { .code = CISTPL_CODE_##NAME, .name = #NAME, .func = &cis_tuple_func_default, }
#define CIS_TUPLE_WITH_FUNC(NAME, FUNC) \
    (cis_tuple_t) { .code = CISTPL_CODE_##NAME, .name = #NAME, .func = &(FUNC), }

#define CIS_CHECK_SIZE(SIZE, MINIMAL)      \
    do                                     \
    {                                      \
        int store_size = (SIZE);           \
        if ((store_size) < (MINIMAL))      \
            return SDMMC_ERR_INVALID_SIZE; \
    } while (0)
#define CIS_CHECK_UNSUPPORTED(COND)         \
    do                                      \
    {                                       \
        if (!(COND))                        \
...
sdmmc_err_t sdmmc_io_reset(sdmmc_card_t *card)
{
    ...
    else if (err == SDMMC_ERR_NOT_FOUND)
    {
        SDMMC_LOGD(TAG, "%s: card not present", __func__);
        return err;
    }
    else if (err != SDMMC_OK)
    {
        SDMMC_LOGE(TAG, "%s: unexpected return: 0x%x", __func__, err);
        return err;
    }
    return SDMMC_OK;
}
...
sdmmc_err_t sdmmc_init_io(sdmmc_card_t *card)
{
    ...
    if (err != SDMMC_OK)
    {
        SDMMC_LOGD(TAG, "%s: io_send_op_cond (1) returned 0x%x; not IO card", __func__, err);
        card->is_sdio = 0;
        card->is_mem = 1;
    }
    else
    {
        ...
        {
            SDMMC_LOGD(TAG, "%s: IO-only card", __func__);
            card->is_mem = 0;
        }
        card->num_io_functions = SD_IO_OCR_NUM_FUNCTIONS(card->ocr);
        SDMMC_LOGD(TAG, "%s: number of IO functions: %d", __func__, card->num_io_functions);
        ...
        if (err != SDMMC_OK)
        {
            SDMMC_LOGE(TAG, "%s: sdmmc_io_send_op_cond (1) returned 0x%x", __func__, err);
            return err;
        }
        err = sdmmc_io_enable_int(card);
        if (err != SDMMC_OK)
        {
            SDMMC_LOGD(TAG, "%s: sdmmc_enable_int failed (0x%x)", __func__, err);
        }
    }
    return SDMMC_OK;
}
...
sdmmc_err_t sdmmc_init_io_bus_width(sdmmc_card_t *card)
{
    ...
    if (card->host.flags & SDMMC_HOST_FLAG_4BIT)
    {
        ...
        if (err != SDMMC_OK)
        {
            SDMMC_LOGE(TAG, "%s: sdmmc_io_rw_direct (read SD_IO_CCCR_CARD_CAP) returned 0x%0x", __func__, err);
            return err;
        }
        SDMMC_LOGD(TAG, "IO card capabilities byte: %02x", card_cap);
            ...
            (card_cap & CCCR_CARD_CAP_4BLS))
        {
            ...
            if (err != SDMMC_OK)
            {
                SDMMC_LOGE(TAG, "%s: sdmmc_io_rw_direct (write SD_IO_CCCR_BUS_WIDTH) returned 0x%0x", __func__, err);
                return err;
            }
...
sdmmc_err_t sdmmc_io_enable_hs_mode(sdmmc_card_t *card)
{
    ...
    if (err != SDMMC_OK)
    {
        SDMMC_LOGD(TAG, "%s: sdmmc_io_rw_direct returned 0x%x", __func__, err);
        return err;
    }

    SDMMC_LOGD(TAG, "%s: CCCR_HIGHSPEED=0x%02x", __func__, val);
...
sdmmc_err_t sdmmc_io_send_op_cond(sdmmc_card_t *card, uint32_t ocr, uint32_t *ocrp)
{
    ...
    for (size_t i = 0; i < 100; i++)
    {
        ...
        SDMMC_DELAY(SDMMC_IO_SEND_OP_COND_DELAY_MS);
    }
                               ...
                               uint32_t reg, uint32_t arg, uint8_t *byte)
{
    ...
    if (err != SDMMC_OK)
    {
        SDMMC_LOGV(TAG, "%s: sdmmc_send_cmd returned 0x%x", __func__, err);
        return err;
    }
                               ...
                               uint32_t addr, uint8_t *out_byte)
{
    ...
    if (ret != SDMMC_OK)
    {
        SDMMC_LOGE(TAG, "%s: sdmmc_io_rw_direct (read 0x%x) returned 0x%x", __func__, addr, ret);
    }
    return ret;
}
                                ...
                                uint32_t addr, uint8_t in_byte, uint8_t *out_byte)
{
    ...
    if (ret != SDMMC_OK)
    {
        SDMMC_LOGE(TAG, "%s: sdmmc_io_rw_direct (write 0x%x) returned 0x%x", __func__, addr, ret);
        return ret;
    }
    if (out_byte != NULL)
    {
...
sdmmc_err_t sdmmc_io_wait_int(sdmmc_card_t *card, TickType_t timeout_ticks)
{
...
{
    SDMMC_LOG_BUFFER_HEXDUMP("CIS", buffer, buffer_size, ESP_LOG_DEBUG);
    if (!fp)
        fp = stdout;
    ...
    do
    {
        ...
        if (ret != SDMMC_OK)
            return ret;
        cis += 2 + size;
        if (tuple && tuple->code == CISTPL_CODE_END)
            break;
...
sdmmc_err_t sdmmc_io_get_cis_data(sdmmc_card_t *card, uint8_t *out_buffer, size_t buffer_size, size_t *inout_cis_size)
{
    ...
    } while (!end_tuple_found && (size_t)buffer_offset < max_reading);

    if (end_tuple_found)
    {
        *inout_cis_size = cur_cis_offset;
        if ((size_t)cur_cis_offset > buffer_size)
        {
            return SDMMC_ERR_INVALID_SIZE;
        }
        else
