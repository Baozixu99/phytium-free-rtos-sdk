...
DSTATUS sata_pcie_disk_status(BYTE pdrv /* Physical drive nmuber to identify the drive */
)
{
...
static void PCieIntxInit(FPcieEcam *instance_p)
{
    ...
    InterruptInstall(FPCIE_ECAM_INTA_IRQ_NUM, (IrqHandler)FPcieEcamIntxIrqHandler,
                     instance_p, "pcieInta");
...
static int FSataInit(void)
{
    ...
    for (host = 0; host < pcie->scans_bdf_count; host++)
    {
        ...
        if (class == class_code)
        {
            ...
            FPcieEcamReadConfigSpace(pcie, bus, device, function,
                                     FPCIE_CCR_BAR_ADDR5_REGS, (u32 *)&bar_addr);
            ...
            FSATA_DEBUG("sata_device[%d].config.base_addr = 0x%x.\n", sata_host_count,
                        sata_device[sata_host_count].config.base_addr);
            sata_host_count++;
...
DSTATUS sata_pcie_disk_initialize(BYTE pdrv /* Physical drive nmuber to identify the drive */
)
{
    DSTATUS status = STA_NOINIT;
    static u8 flag = 0;
...
DRESULT sata_pcie_disk_read(BYTE pdrv, /* Physical drive nmuber to identify the drive */
...
DRESULT sata_pcie_disk_write(BYTE pdrv, /* Physical drive nmuber to identify the drive */
...
DRESULT sata_pcie_disk_ioctl(BYTE pdrv, /* Physical drive nmuber (0..) */
...
static const ff_diskio_driver_t sata_pcie_disk_drv = {.init = &sata_pcie_disk_initialize,
