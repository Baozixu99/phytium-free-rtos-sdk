 ...
 *
 *
 * FilePath: pusb2_usb_disk_bench_example.c
...
enum
{
    PUSB2_USB_DISK_RW_SUCCESS = 0,
    PUSB2_USB_DISK_RW_FAIL    = 1,
};
...
#define FUSB_DEBUG_TAG "PUSB2-USB-DISK"
...
/*****************************************************************************/
static void PUSB2UsbMscBenchTask(void *args)
{
    int ret;
    u8 id = (u8)usb_id;
...
task_exit:
    ret = (ret == 0) ? PUSB2_USB_DISK_RW_SUCCESS : PUSB2_USB_DISK_RW_FAIL;
    xQueueSend(xQueue, &ret, 0);
    vTaskDelete(NULL);
}

BaseType_t FFreeRTOSRunPUSB2DiskBench(u32 id, const char *devname)
{
    BaseType_t ret = pdPASS;
    int task_res = PUSB2_USB_DISK_RW_SUCCESS;
    ...
    ret = xTaskCreate((TaskFunction_t)PUSB2UsbMscBenchTask,
                      (const char *)"PUSB2UsbMscBenchTask",
                      (uint16_t)4096,
                      (void *)devname,
...
exit:
    ...
    if (task_res != PUSB2_USB_DISK_RW_SUCCESS)
    {
        printf("%s@%d: PUSB2 usb disk read/write example [failure], task_res = %d\r\n", __func__, __LINE__, task_res);
        return pdFAIL;
    }
    else
    {
        printf("%s@%d: PUSB2 usb disk read/write example [success].\r\n", __func__, __LINE__);
        return pdTRUE;
    }
}
