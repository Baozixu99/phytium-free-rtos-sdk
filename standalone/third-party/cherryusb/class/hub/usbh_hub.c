...
static int parse_hub_ss_descriptor(struct usb_hub_ss_descriptor *desc, uint16_t length)
{
    ...
    if (desc->bLength < USB_SIZEOF_HUB_SS_DESC) {
        ...
        USB_LOG_INFO("SuperSpeed Hub Descriptor:\r\n");
        USB_LOG_INFO("bLength: 0x%02x             \r\n", desc->bLength);
        USB_LOG_INFO("bDescriptorType: 0x%02x     \r\n", desc->bDescriptorType);
        USB_LOG_INFO("bNbrPorts: 0x%02x           \r\n", desc->bNbrPorts);
        USB_LOG_INFO("wHubCharacteristics: 0x%04x \r\n", desc->wHubCharacteristics);
...
static int usbh_hub_connect(struct usbh_hubport *hport, uint8_t intf)
{
    ...
    USB_LOG_RAW("Register HUB Class:%s [%s] for usb-%d\r\n", hport->config.intf[intf].devname, speed_table[hport->speed], hport->bus->busid);
...
static int usbh_hub_disconnect(struct usbh_hubport *hport, uint8_t intf)
{
    ...
    if (hub) {
        ...
        if (hport->config.intf[intf].devname[0] != '\0') {
            USB_LOG_RAW("Unregister HUB Class:%s [%s] for usb-%d\r\n", hport->config.intf[intf].devname, speed_table[hport->speed], hport->bus->busid);
        }
