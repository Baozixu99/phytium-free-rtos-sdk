...
#define MMC_CMD1_RETRY_TIMES (10U)
#endif
#ifndef MMC_CMD13_RETRY_TIMES
#define MMC_CMD13_RETRY_TIMES (10U)
#endif
#ifndef MMC_CARD_ACCESS_WAIT_IDLE_TIMEOUT
#define MMC_CARD_ACCESS_WAIT_IDLE_TIMEOUT (10U)
#endif
...
static void MMC_DecodeExtendedCsd(mmc_card_t *card, uint32_t *rawExtendedCsd)
{
    ...
    volatile uint8_t *buffer = (uint8_t *)rawExtendedCsd;
...
static status_t MMC_TestDataBusWidth(mmc_card_t *card, mmc_data_bus_width_t width)
{
    ...
    if (card->host->config.hostType == kSDMMCHOST_TYPE_FSDIF_V2)
    {
        return kStatus_Success; /* SDIF V2 do not support CMD19 */
    }
#endif
...
#if defined(CONFIG_FSL_SDMMC_USE_FSDIF) || defined(CONFIG_FSL_SDMMC_USE_FSDIF_V2)
    card->busClock_Hz = SDMMCHOST_SetCardClock(card->host, hs400Freq);
#else
