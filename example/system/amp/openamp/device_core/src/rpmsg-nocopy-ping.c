...
#define     NO_COPY_DRIVER_DEBUG_TAG "    DRIVER_04"
#define     NO_COPY_DRIVER_DEBUG_I(format, ...) FT_DEBUG_PRINT_I( NO_COPY_DRIVER_DEBUG_TAG, format, ##__VA_ARGS__)
#define     NO_COPY_DRIVER_DEBUG_W(format, ...) FT_DEBUG_PRINT_W( NO_COPY_DRIVER_DEBUG_TAG, format, ##__VA_ARGS__)
#define     NO_COPY_DRIVER_DEBUG_E(format, ...) FT_DEBUG_PRINT_E( NO_COPY_DRIVER_DEBUG_TAG, format, ##__VA_ARGS__)
...
static int rpmsg_check_rcv_msg(struct rpmsg_rcv_msg *rcv_msg, uint32_t exp_num)
{
	...
	for (i = 0; i < (int)r_payload->size; i++) {
		if (r_payload->data[i] != 0xA5) {
			NO_COPY_DRIVER_DEBUG_I("Data corruption at index %d\r\n", i);
			ret = RPMSG_ERR_PARAM;
			goto out;
		}
	}
				       ...
				       const char *name, uint32_t dest)
{
	...
	if (strcmp(name, RPMSG_SERVICE_NAME))
		NO_COPY_DRIVER_DEBUG_E("Unexpected name service %s.\r\n", name);
	else
...
static int app(struct rpmsg_device *rdev, void *priv)
{
	...
	NO_COPY_DRIVER_DEBUG_I(" 1 - Send data to remote core, retrieve the echo");
	NO_COPY_DRIVER_DEBUG_I(" and validate its integrity ..\r\n");
...
int rpmsg_nocopy_ping(struct rpmsg_device *rdev, void *priv)
{
	...
	if (ret != 0)
    {
        NO_COPY_DRIVER_DEBUG_E("Rpmsg_nocopy_ping application error,code:0x%x",ret);
        return ret;
    }
