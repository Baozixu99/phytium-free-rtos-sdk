...
- 新建 `tftpboot`目录, `/mnt/d/tftpboot`, 此目录应与项目编译脚本makefile中的USR_BOOT_DIR一致, 并确保 tftpboot 目录有执行权限`chmod 777 /**/tftpboot`
...
- 修改主机`tftp`配置，指向`tftpboot`目录，修改/etc/default/tftpd-hpa
...
- 根据需求，配置在不同芯片下32位或64位的编译模式，这里以`飞腾腾珑E2000D`的32位为例，输入以下指令
    ...
    ```
    make load_kconfig LOAD_CONFIG_NAME=pe2202_aarch32_demo_serial
    ```
...
- 出现如下信息，表示编译成功，编译结果.elf与.bin文件已经保存至tftpboot文件夹内
...
- 可以看到生成了汇编文件以及.bin文件，但是如果需要添加 `/mnt/d/tftpboot/`,您可以在工程目录下的 `makeflie` 文件下的 image: 操作中，追加一条 `cp ./$(IMAGE_OUT_NAME).bin $(USR_BOOT_DIR)/baremetal.bin` 命令，或者手动复制。
...
- 随后烧录`tftpboot`文件夹下的文件到开发板，输入以下指令
...
- 复制`~/standalone-sdk/example/system/template`目录，作为 baremetal 应用工程
...
- 将`BIN`文件或者`ELF`文件复制到`tftpboot`目录
  ...
  ```
  $ cp ./baremetal.bin /mnt/d/tftpboot
  $ cp ./baremetal.elf /mnt/d/tftpboot
  ```
