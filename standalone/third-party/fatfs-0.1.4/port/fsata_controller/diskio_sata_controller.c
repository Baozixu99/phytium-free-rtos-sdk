...
DSTATUS sata_disk_status(BYTE pdrv /* Physical drive nmuber to identify the drive */
)
{
...
static int FSataInit(void)
{
    ...
    for (instance_id = 0; instance_id < FSATA_NUM; instance_id++)
    {
        ...
        if (config_p != NULL)
        {
            ...
            FSATA_DEBUG("Plat ahci host[%d] base_addr = 0x%x", instance_id,
                        sata_device[instance_id].config.base_addr);
            FSATA_DEBUG("Plat ahci host[%d] irq_num = %d", instance_id,
                        sata_device[instance_id].config.irq_num);
        }
        else
        {
            continue;
        ...
        if (FSATA_SUCCESS != ret)
        {
            ...
            FSATA_DEBUG("FSataAhciInit sata failed, ret: 0x%x.", ret);
            continue;
        }
        ...
        for (port = 0; port < instance_p->n_ports; port++)
        {
            ...
            ret = FSataAhciPortStart(instance_p, port,
                                     (uintptr)mem + PALIGN_UP(FSATA_AHCI_PORT_PRIV_DMA_SZ, ADDR_ALIGNMENT) *
                                                        port_mem_count);
            port_mem_count++;
            if (FSATA_SUCCESS != ret)
            {
...
DSTATUS sata_disk_initialize(BYTE pdrv /* Physical drive nmuber to identify the drive */
)
{
    DSTATUS status = STA_NOINIT;
...
DRESULT sata_disk_read(BYTE pdrv,    /* Physical drive nmuber to identify the drive */
...
DRESULT sata_disk_write(BYTE pdrv, /* Physical drive nmuber to identify the drive */
...
)
{
    ...
    if (FSATA_SUCCESS != err)
    {
        FSATA_ERROR("Write sata controller sector [%d-%d] failed: 0x%x.", sector,
                    sector + count, err);
        status = RES_ERROR;
    }
...
DRESULT sata_disk_ioctl(BYTE pdrv, /* Physical drive nmuber (0..) */
...
static const ff_diskio_driver_t sata_disk_drv = {.init = &sata_disk_initialize,
