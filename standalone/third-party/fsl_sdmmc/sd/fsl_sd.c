...
static void SD_DumpCard(sd_card_t *card)
{
    ...
    capcity = (uint64_t)card->blockSize * (uint64_t)card->blockCount;
    if (capcity > (uint64_t)SZ_2G) {
        SDMMC_LOG("  Size: %d GB\r\n",
                (uint32_t)(((uint64_t)card->blockSize * (uint64_t)card->blockCount) / (uint64_t)SZ_1G));
    } else if (capcity > (uint64_t)SZ_2M) {
        SDMMC_LOG("  Size: %d MB\r\n",
                (uint32_t)(((uint64_t)card->blockSize * (uint64_t)card->blockCount) / (uint64_t)SZ_1M));
    } else if (capcity > (uint64_t)SZ_2K) {
        SDMMC_LOG("  Size: %d KB\r\n",
                (uint32_t)(((uint64_t)card->blockSize * (uint64_t)card->blockCount) / (uint64_t)SZ_1K));
    } else {
        SDMMC_LOG("  Size: %d Bytes\r\n", capcity);
    }
