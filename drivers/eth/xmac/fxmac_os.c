...
FError FXmacSgsend(FXmacOs *instance_p, struct pbuf *p)
{
    ...
    for (q = p, txbd = txbdset; q != NULL; q = q->next)
    {
        ...
        if (instance_p->buffer.tx_pbufs_storage[bdindex])
        {
            ...
            FXMAC_OS_XMAC_PRINT_I("PBUFS not available bdindex is %d \r\n", bdindex);
            FXMAC_OS_XMAC_PRINT_I("instance_p->buffer.tx_pbufs_storage[bdindex] %p \r\n", instance_p->buffer.tx_pbufs_storage[bdindex]);
            return ERR_GENERAL;
        }
