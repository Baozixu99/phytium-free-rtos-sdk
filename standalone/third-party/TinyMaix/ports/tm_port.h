...
/******************************* PORT CONFIG  ************************************/
#define TM_ARCH           CONFIG_TINYMAIX_USING_ARCK_TYPE
#define TM_OPT_LEVEL      CONFIG_TINYMAIX_USING_OPTION_LEVEL
#define TM_MDL_TYPE       CONFIG_TINYMAIX_USING_MODULE_TYPE
//enable if your chip don't have FPU, may speed up 1/3, but decrease accuracy
#define TM_FASTSCALE      (0)
//use local math func (like exp()) to avoid libm
#define TM_LOCAL_MATH     (0)
//enable mdl stat functions
#define TM_ENABLE_STAT    (1)
//max channel num //used if INT8 mdl  //cost TM_MAX_CSIZE*4 Byte
#define TM_MAX_CSIZE      (1000)
//max kernel_size   //cost TM_MAX_KSIZE*4 Byte
#define TM_MAX_KSIZE      (5 * 5)
//max kernel_size*channels //cost TM_MAX_KSIZE*sizeof(mtype_t) Byte
...
#define TM_TEST_DEBUG_TAG "TM"
#define TM_PRINTF(format, ...) \
    FT_DEBUG_PRINT_I(TM_TEST_DEBUG_TAG, format, ##__VA_ARGS__)
#define TM_DBG(format, ...) FT_DEBUG_PRINT_D(TM_TEST_DEBUG_TAG, format, ##__VA_ARGS__)

/******************************* DBG TIME CONFIG  ************************************/
#include <sys/time.h>
#include <time.h>
#define TM_GET_US() ((uint32_t)((uint64_t)GenericTimerRead(GENERIC_TIMER_ID0) / 50))
