...
#define NVME_CMD_DEBUG_TAG "NVME_CMD"
#define NVME_CMD_DEBUG_I(format, ...) FT_DEBUG_PRINT_I(NVME_CMD_DEBUG_TAG, format, ##__VA_ARGS__)
#define NVME_CMD_DEBUG_W(format, ...) FT_DEBUG_PRINT_W(NVME_CMD_DEBUG_TAG, format, ##__VA_ARGS__)
#define NVME_CMD_DEBUG_E(format, ...) FT_DEBUG_PRINT_E(NVME_CMD_DEBUG_TAG, format, ##__VA_ARGS__)
#define NVME_CMD_DEBUG_D(format, ...) FT_DEBUG_PRINT_D(NVME_CMD_DEBUG_TAG, format, ##__VA_ARGS__)
...
static void sys_write32(uint32_t data, uintptr addr)
{
...
void nvme_completion_print(const struct nvme_completion *cpl)
{
	...
	NVME_CMD_DEBUG_D("%s (%02x/%02x) crd:%x m:%x dnr:%x p:%d "
		"sqid:%d cid:%d cdw0:%x\n",
...
void nvme_cmd_init(struct nvme_controller *nvme_ctrlr)
{
	int idx;

	sys_dlist_init(&nvme_ctrlr->free_request);
	sys_dlist_init(&nvme_ctrlr->pending_request);
	sys_dlist_init(&nvme_ctrlr->free_prp_list);

	for (idx = 0; idx < NVME_REQUEST_AMOUNT; idx++) {
		sys_dlist_append(&nvme_ctrlr->free_request, &nvme_ctrlr->request_pool[idx].node);
	}

	for (idx = 0; idx < CONFIG_NVME_PRP_LIST_AMOUNT; idx++) {
		sys_dlist_append(&nvme_ctrlr->free_prp_list, &nvme_ctrlr->prp_list_pool[idx].node);
	}
}

static struct nvme_prp_list *nvme_prp_list_alloc(struct nvme_cmd_qpair *qpair)
{
	sys_dnode_t *node;
	struct nvme_controller *nvme_ctrlr = qpair->ctrlr ;
	node = sys_dlist_peek_head(&nvme_ctrlr->free_prp_list);
	if (!node) {
		NVME_CMD_DEBUG_E("Could not allocate PRP list");
		return NULL;
	}
...
static void nvme_prp_list_free(struct nvme_controller *nvme_ctrlr ,struct nvme_prp_list *prp_list)
{
	memset(prp_list, 0, sizeof(struct nvme_prp_list));
	sys_dlist_append(&nvme_ctrlr->free_prp_list, &prp_list->node);
}

void nvme_cmd_request_free(struct nvme_request *request)
{
	struct nvme_controller *nvme_ctrlr = request->qpair->ctrlr ;
	if (sys_dnode_is_linked(&request->node)) {
		sys_dlist_remove(&request->node);
	}

	if (request->prp_list != NULL) {
		nvme_prp_list_free(nvme_ctrlr,request->prp_list);
	}

	memset(request, 0, sizeof(struct nvme_request));
	sys_dlist_append(&nvme_ctrlr->free_request, &request->node);
}

struct nvme_request *nvme_cmd_request_alloc(struct nvme_controller *ctrlr)
{
	sys_dnode_t *node;

	node = sys_dlist_peek_head(&ctrlr->free_request);
	if (!node) {
		NVME_CMD_DEBUG_E("Could not allocate request");
		return NULL;
	}
...
{
	struct nvme_controller *nvme_ctrlr = request->qpair->ctrlr ;
	sys_dlist_append(&nvme_ctrlr->pending_request, &request->node);
	request->req_start = TICKS_TO_SECONDS();
				      ...
				      struct nvme_completion *cpl)
{
	...
	if (retry) {
		NVME_CMD_DEBUG_D("CMD will be retried");
		request->qpair->num_retries++;
	}

	if (error &&
	    (!retriable || (request->retries >= CONFIG_NVME_RETRY_COUNT))) {
		NVME_CMD_DEBUG_D("CMD error");
		request->qpair->num_failures++;
	}

	if (cpl->cid != request->cmd.cdw0.cid) {
		NVME_CMD_DEBUG_E("cpl cid:%d != cmd cid:%d ",cpl->cid,request->cmd.cdw0.cid);
	}

	if (retry) {
		NVME_CMD_DEBUG_D("Retrying CMD");
...
void nvme_cmd_qpair_process_completion(struct nvme_cmd_qpair *qpair)
{
	...
	if (qpair->num_intr_handler_calls == 0 && qpair->phase == 0) {
		NVME_CMD_DEBUG_W("Phase wrong for first interrupt call.");
	}
	...
	while (1) {
		...
		if (NVME_STATUS_GET_P(status) != NVME_STATUS_GET_P(cpl.status)) {
			NVME_CMD_DEBUG_W("Phase unexpectedly inconsistent");
		}

		if (cpl.cid < NVME_REQUEST_AMOUNT) {
			request = &nvme_ctrlr->request_pool[cpl.cid];
		} else {
			request = NULL;
		}
		...
		if (request != NULL) {
			...
			NVME_CMD_DEBUG_E("cpl (cid = %u) does not map to cmd", cpl.cid);
		}
			 ...
			 uint32_t id)
{
	...
	ctrlr->interrupt_setup_fun(qpair->vector,nvme_cmd_qpair_msi_handler,qpair,1) ;


	NVME_CMD_DEBUG_D("CMD Qpair created ID %u, %u entries - cmd/cpl addr "
					...
					int n_prp)
{
	...
	prp_list = nvme_prp_list_alloc(qpair);
	if (prp_list == NULL) {
		return -ENOMEM;
	}
	...
	request->cmd.dptr.prp2 =
		(uint64_t)sys_cpu_to_le64((uintptr_t)&prp_list->prp);
				    ...
				    struct nvme_request *request)
{
	...
	case NVME_REQUEST_VADDR:
		...
		if (request->payload_size > qpair->ctrlr->max_xfer_size) {
			NVME_CMD_DEBUG_E("VADDR request's payload too big");
			NVME_CMD_DEBUG_E("payload_size: 0x%x, max_xfer_size: 0x%x",request->payload_size,qpair->ctrlr->max_xfer_size);
		...
		if (n_prp <= 2) {
			request->cmd.dptr.prp1 =
				(uint64_t)sys_cpu_to_le64((uintptr_t)request->payload);
			if (n_prp == 2) {
