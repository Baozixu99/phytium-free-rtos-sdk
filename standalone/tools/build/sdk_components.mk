...
LINK_FILE_NAME ?= $(CONFIG_ARCH_EXECUTION_STATE)_ram
LINK_FILE_OBJECT ?= $(CONFIG_ARCH_EXECUTION_STATE)_ram.ld.o
LINK_FILE_PATH ?= $(SDK_DIR)/tools/build/ld


LIBS_OBJS = $(BUILD_OUT_PATH)/$(LINK_FILE_OBJECT)

$(BUILD_OUT_PATH)/$(LINK_FILE_OBJECT):
	@$(MAKE) -s -C $(LINK_FILE_PATH) SDK_DIR="$(SDK_DIR)" PROJECT_DIR="$(PROJECT_DIR)" LD_NAME="$(LINK_FILE_NAME)" BUILD_OUT_PATH="$(BUILD_OUT_PATH)" USER_INCLUDE_PATH="$(USER_INCLUDE)"  all
...
# sdk 自定义静态文件库
SOURCE_DEFINED_LIBS ?= 
ifdef CONFIG_CHERRY_USB_PORT_STATIC_LIB
