...
sdmmc_err_t sdmmc_send_cmd(sdmmc_card_t *card, sdmmc_command_t *cmd)
{
    ...
    SDMMC_LOGV(TAG, "sending cmd slot=%d op=%d arg=%x flags=%x data=%p blklen=%d datalen=%d timeout=%d",
    ...
    if (err != 0)
    {
        SDMMC_LOGD(TAG, "cmd=%d, sdmmc_req_run returned 0x%x", cmd->opcode, err);
        return err;
    }
    int state = MMC_R1_CURRENT_STATE(cmd->response);
    SDMMC_LOGV(TAG, "cmd response %08x %08x %08x %08x err=0x%x state=%d",
               cmd->response[0],
               cmd->response[1],
               cmd->response[2],
               cmd->response[3],
...
sdmmc_err_t sdmmc_send_app_cmd(sdmmc_card_t *card, sdmmc_command_t *cmd)
{
    ...
    SDMMC_LOGI(TAG, "cmd to be send %d", cmd->opcode);
    ...
    {
        SDMMC_LOGW(TAG, "card doesn't support APP_CMD");
...
sdmmc_err_t sdmmc_send_cmd_go_idle_state(sdmmc_card_t *card)
{
    ...
    if (host_is_spi(card))
    {
         ...
         */
        (void)err;
        SDMMC_DELAY(SDMMC_GO_IDLE_DELAY_MS);
...
sdmmc_err_t sdmmc_send_cmd_send_if_cond(sdmmc_card_t *card, uint32_t ocr)
{
    ...
    if (response != pattern)
    {
        SDMMC_LOGE(TAG, "%s: received=0x%x expected=0x%x", __func__, response, pattern);
...
sdmmc_err_t sdmmc_send_cmd_send_op_cond(sdmmc_card_t *card, uint32_t ocr, uint32_t *ocrp)
{
    ...
    for (; nretries != 0; --nretries)
    {
        ...
        if (!card->is_mmc)
        { /* SD mode */
            SDMMC_LOGD(TAG, "SD mode");
            cmd.opcode = SD_APP_OP_COND;
            err = sdmmc_send_app_cmd(card, &cmd);
        }
        else
        { /* MMC mode */
            SDMMC_LOGD(TAG, "MMC mode");
        ...
        if (err != SDMMC_OK)
        {
            if (--err_cnt == 0)
            {
                SDMMC_LOGD(TAG, "%s: sdmmc_send_app_cmd err=0x%x", __func__, err);
                return err;
            }
            else
            {
                SDMMC_LOGV(TAG, "%s: ignoring err=0x%x", __func__, err);
                continue;
            }
        }
...
sdmmc_err_t sdmmc_send_cmd_set_blocklen(sdmmc_card_t *card, sdmmc_csd_t *csd)
{
    sdmmc_command_t cmd = {
        ...
        .arg = csd->sector_size,
        .flags = SCF_CMD_AC | SCF_RSP_R1};
    SDMMC_LOGD(TAG, "set block len: sector size: 0x%x", csd->sector_size);
    return sdmmc_send_cmd(card, &cmd);
}
...
sdmmc_err_t sdmmc_send_cmd_send_csd(sdmmc_card_t *card, sdmmc_csd_t *out_csd)
{
    ...
    sdmmc_command_t cmd = {
        ...
        .flags = is_spi ? (SCF_CMD_READ | SCF_CMD_ADTC | SCF_RSP_R1) : (SCF_CMD_AC | SCF_RSP_R2),
...
{
    size_t datalen = 8;
    uint32_t *buf = (uint32_t *)sdmmc_port_align_malloc(datalen, TRUE);
    if (buf == NULL)
    {
        return SDMMC_ERR_NO_MEM;
    }
    SDMMC_LOGI(TAG, "send scr buf: %p, datalen: %d", buf, datalen);
    sdmmc_command_t cmd = {
        .data = buf,
        .datalen = datalen,
        .blklen = datalen,
...
/* set number of blocks in next transcation */
sdmmc_err_t sdmmc_set_blockcount(sdmmc_card_t *card, size_t block_count)
{
    sdmmc_command_t cmd = {
                                ...
                                size_t start_block, size_t block_count)
{
    ...
    if (SDMMC_PTR_DMA_CAP(src) && (intptr_t)src % 4 == 0)
    {
    ...
    else
    {
        ...
        // DMA-capable buffer.
        void *tmp_buf = sdmmc_port_align_malloc(block_size, TRUE);
        if (tmp_buf == NULL)
        {
            return SDMMC_ERR_NO_MEM;
        }
        ...
        for (size_t i = 0; i < block_count; ++i)
        {
            ...
            if (err != SDMMC_OK)
            {
                SDMMC_LOGD(TAG, "%s: error 0x%x writing block %d+%d",
                           __func__, err, start_block, i);
                break;
            }
        }
        sdmmc_port_align_free(tmp_buf);
    }
    return err;
}
                                    ...
                                    size_t start_block, size_t block_count)
{
    if (start_block + block_count > (size_t)card->csd.capacity)
    {
        ...
        SDMMC_LOGI(TAG, "start: %d, cnt: %d, capacity: %d", start_block, block_count, (size_t)card->csd.capacity);
    ...
    /* for multi-block transcation, send CMD-23 to announce block num, this is optional */
    if (block_count > 1)
    {
        err = sdmmc_set_blockcount(card, block_count);
        if (err != SDMMC_OK)
        {
            SDMMC_LOGE(TAG, "%s: sdmmc_set_blockcount returned 0x%x", __func__, err);
            return err;
        }  
    } 
    ...
    /* stop transmission for multi-block write, CMD12 is no supported for mmc card */
    if ((FALSE == card->is_mmc) && (block_count > 1))
    {
        err = sdmmc_send_stop_transmission(card);
        if (err != SDMMC_OK)
        {
            SDMMC_LOGE(TAG, "%s: send_stop_transmission returned 0x%x", __func__, err);
            return err;
        }
    }
                               ...
                               size_t start_block, size_t block_count)
{
    sdmmc_err_t err = SDMMC_OK;
    size_t block_size = card->csd.sector_size;
    if (SDMMC_PTR_DMA_CAP(dst) && (intptr_t)dst % 4 == 0)
    {
                                   ...
                                   size_t start_block, size_t block_count)
{
    ...
    /* stop transmission for multi-block read, CMD12 is no supported for mmc card */
    if ((FALSE == card->is_mmc) && (block_count > 1))
    {
        err = sdmmc_send_stop_transmission(card);
        if (err != SDMMC_OK)
...
sdmmc_err_t sdmmc_send_cmd_switch(sdmmc_card_t *card)
{
    ...
    uint32_t *buf = (uint32_t *)sdmmc_port_align_malloc(datalen, FALSE);
    if (NULL == buf)
    {
        return SDMMC_ERR_NO_MEM;
    }
