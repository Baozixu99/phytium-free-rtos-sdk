/* This example demonstrates:
   ...
   Auto reload timer will stop after it has executed 5 times which period is 1000, when the one shot timer is run after 9333 ticks, it will reset auto reload timer.
 */
#include "FreeRTOS.h"
#include "task.h"
#include "queue.h"
...
#define ONE_SHOT_TIMER_PERIOD       (pdMS_TO_TICKS(9000UL))
#define AUTO_RELOAD_TIMER_PERIOD    (pdMS_TO_TICKS(2000UL))

#define EXAMPLE_TIMEOUT             (pdMS_TO_TICKS(20000UL))
...
{   
    ...
    /* The count of the number of times this software timer has expired is stored in the timer's ID.
     ...
     * Compare the handle passed into this function with xOneShotTimer to determine if it was the one-shot or auto-reload timer that expired,
       then output a string to show the time at which the callback was executed.
     */
    if (xTimer == xOneShotTimer)
    {
        vPrintStringAndNumber("One-shot timer callback executing", xTimeNow);

        /* If auto-reload timer id is not still 2, auto-reload timer stop failed. */
        ulExecutionCount = (uint32_t)(uintptr)pvTimerGetTimerID(xAutoReloadTimer);
        if (ulExecutionCount != 2)
        {
        ...
        {
            vPrintString("Delete one-shot timer reset task.");
        }
    }
    else
    {
        /* xTimer did not equal xOneShotTimer, so it must be the auto-reload timer that expired. */
        vPrintStringAndNumber("Auto-reload timer callback executing", xTimeNow);

        if (ulExecutionCount == 2)
        {
            /* Stop the auto-reload timer after it has executed 3 times.
             * This callback function executes in the context of the RTOS daemon task so must not call any functions that might 
               place the daemon task into the Blocked state.
             * Therefore a block time of 0 is used.
             */
            xTimerStop(xTimer, 0);
            vPrintString("Auto-reload timer stopped.");
        }
        else if (ulExecutionCount == 4)
        {   
            xTimerStop(xTimer, 0);
            vPrintString("Auto-reload timer stopped again.");
            ...
            {
                vPrintString("Delete auto-reload timer reset tasks.");
                xQueueSend(xQueue, &task_res, 0);
            }
        }
    }
...
/*-----------------------------------------------------------*/
/* The timer handles are used inside the callback function so this time are given file scope. */
int CreateTimerResetTasks(void)
{
    ...
    xQueue = xQueueCreate(2, sizeof(int)); /* Create Message Queue */
    if (xQueue == NULL)
    {
    ...
    /* Create the auto-reload, storing the handle to the created timer in xAutoReloadTimer. */
    ...
    {
        /* Start the timers, using a block time of 0 (no block time).
...
exit:
    ...
    {
        vPrintf("%s@%d: Software timer set timer id and reset example [failure], task_res = %d \r\n", __func__, __LINE__, task_res);
        return task_res;
    }
    else
    {
        vPrintf("%s@%d: Software timer set timer id and reset example [success]. \r\n", __func__, __LINE__);
        return task_res;
    }
}