...
### FPcieEcamVectorsAllocate

- 分配给定PCI设备的MSI向量。

```c
u8 FPcieEcamVectorsAllocate(FPcieEcam *instance_p, u8 bus, u8 device, u8 function, FPcieMsiVector *vectors)
```

Note:

- 这个函数确定MSI能力并分配所需数量的向量。如果成功，返回分配的MSI向量数量；如果失败，返回0。
...
- `u8 device`: PCI设备号。
- `u8 function`: PCI功能号。
- `FPcieMsiVector *vectors`: 指向FPcieMsiVector结构的指针，用于存储分配的向量。

Output:

- `u8`: 分配的MSI向量数量，如果出错则返回0。
...
- 检索MSI能力基地址并设置MSI。如果成功，返回0；如果出错，返回-1。

Input:

- `FPcieEcam *instance_p`: 指向FPcieEcam实例的指针。
- `FPcieMsiVector *vectors`: 指向包含MSI向量的FPcieMsiVector结构的指针。

Output:

- `u8`: 如果成功，返回0；如果失败，返回-1。


### FPcieEcamVectorsFree
...
- 在GIC中取消初始化设备并释放LPI ID。

Input:

- `FPcieEcam *instance_p`: 指向FPcieEcam实例的指针。
- `FPcieMsiVector *vectors`: 指向包含已分配向量的FPcieMsiVector结构的指针。
