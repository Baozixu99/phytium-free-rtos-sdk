...
#define SDMMC_INIT_STEP(condition, function)                                       \
    ...
    {                                                                              \
        ...
        {                                                                          \
            ...
            if (err != SDMMC_OK)                                                   \
            {                                                                      \
                SDMMC_LOGD(TAG, "%s: %s returned 0x%x", __func__, #function, err); \
                return err;                                                        \
            }                                                                      \
        }                                                                          \
    } while (0);
...
sdmmc_err_t sdmmc_card_init(const sdmmc_host_t *config, sdmmc_card_t *card)
{
    ...
    const bool io_supported = host_support_sdio(card); //true;
    ...
    /* if support io, send CMD5 to check if it is memory card inserted, otherwise, 
       assert card as memory */
    const bool is_mem = io_supported ? card->is_mem : true;
    /* if not support io, assert sdio not support */
    const bool is_sdio = io_supported ? !is_mem : false;

    if (!io_supported)
        card->is_mem = 1; /* assert card as memory card when io not support */

    /* Enable CRC16 checks for data transfers in SPI mode */
    SDMMC_INIT_STEP(is_spi, sdmmc_init_spi_crc);

    /* Use SEND_OP_COND(CMD41) to set up card OCR */
    SDMMC_INIT_STEP(is_mem, sdmmc_init_ocr);

    const bool is_mmc = is_mem && card->is_mmc;
    const bool is_sdmem = is_mem && !is_mmc;

    SDMMC_LOGD(TAG, "%s: card type is %s", __func__,
    ...
    /* essential!!! switch from 1bit to 4bit */
    SDMMC_INIT_STEP(is_sdmem, sdmmc_init_switch_hs);
...
sdmmc_err_t sdmmc_init_tf_card(const sdmmc_host_t *config, sdmmc_card_t *card)
{
    const bool always = true;
    memset(card, 0, sizeof(*card)); /* reset card data */
    card->is_mem = TRUE; /* assert card as memory card when io not support */
    card->is_mmc = FALSE; /* assert card is not un-removeable emmc card */
    card->is_sdio = FALSE;
    memcpy(&card->host, config, sizeof(*config)); /* copy card configs */

    SDMMC_LOGD(TAG, "card type is SD");

    /* Check if host flags are compatible with slot configuration. */
    SDMMC_INIT_STEP(always, sdmmc_fix_host_flags); 
    ...
    /* Use SEND_OP_COND (CMD41) to set up card OCR */
    SDMMC_INIT_STEP(always, sdmmc_init_ocr);

    /* Read the contents of CID register (CMD2)*/
    SDMMC_INIT_STEP(always, sdmmc_init_cid);

    /* Assign RCA (CMD3) */
    SDMMC_INIT_STEP(always, sdmmc_init_rca);

    /* Read and decode the contents of CSD register (CMD9) */
    SDMMC_INIT_STEP(always, sdmmc_init_csd);

    /* Switch the card from stand-by mode to data transfer mode (not needed if
     * SPI interface is used). This is needed to issue SET_BLOCKLEN and
     * SEND_SCR commands. (CMD7)
     */
    SDMMC_INIT_STEP(always, sdmmc_init_select_card);

    /* Get bus width of card (CMD51) */
    SDMMC_INIT_STEP(always, sdmmc_init_sd_scr);

    /* Check current status of card (CMD13) */
    SDMMC_INIT_STEP(always, sdmmc_init_sd_wait_data_ready);

    /* Set bus width (CMD6) */
    SDMMC_INIT_STEP(always, sdmmc_init_sd_bus_width);

    /* Call driver function to change bus width */
    SDMMC_INIT_STEP(always, sdmmc_init_host_bus_width);

    /* SD memory cards:
     * Set block len for SDSC cards to 512 bytes (same as SDHC)
     * Read SCR
     * Wait to enter data transfer state (CMD16)
     */
    SDMMC_INIT_STEP(always, sdmmc_init_sd_blocklen);    
    ...
    /* Call driver function use card->max_freq_khz frequency. */
    SDMMC_INIT_STEP(always, sdmmc_init_host_frequency);

    return SDMMC_OK;
}

sdmmc_err_t sdmmc_init_emmc(const sdmmc_host_t *config, sdmmc_card_t *card)
{
    const bool always = true;
    memset(card, 0, sizeof(*card)); /* reset card data */
    card->is_mem = TRUE; /* assert card as memory card when io not support */
    card->is_mmc = TRUE; /* assert card is not un-removeable emmc card */
    card->is_sdio = FALSE;
    memcpy(&card->host, config, sizeof(*config)); /* copy card configs */

    SDMMC_LOGD(TAG, "card type is eMMC");
    ...
    /* Use MMC_SEND_OP_COND (CMD1) to set up card OCR */
    SDMMC_INIT_STEP(always, sdmmc_init_ocr);
    ...
    /* Decode the contents of mmc CID register */
    SDMMC_INIT_STEP(always, sdmmc_init_mmc_decode_cid);    
    ...
    /* MMC cards: read CXD (CMD8) */
    SDMMC_INIT_STEP(always, sdmmc_init_mmc_read_ext_csd);  

    /* Switch bus width and call driver function to change bus width (CMD6) */
    SDMMC_INIT_STEP(always, sdmmc_init_mmc_bus_width);
    SDMMC_INIT_STEP(always, sdmmc_init_host_bus_width);
