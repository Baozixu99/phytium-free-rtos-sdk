...
sdmmc_err_t sdmmc_init_card_hs_mode(sdmmc_card_t *card)
{
    ...
    {
        SDMMC_LOGD(TAG, "%s: host supports HS mode, but card doesn't", __func__);
...
{
    /* if max freq of card exceed host, host configuration must change, otherwise card init will failed */
    if (card->max_freq_khz > card->host.max_freq_khz)
    {
        SDMMC_LOGE(TAG, "max freq of card %dkHz shall <= max freq of host %dkHz", 
                    card->max_freq_khz, card->host.max_freq_khz);
        return SDMMC_FAIL;
    }

    SDMMC_ASSERT(card->max_freq_khz <= card->host.max_freq_khz);
    ...
    const uint32_t freq_values[] = {
        ...
        SDMMC_FREQ_26M,
        SDMMC_FREQ_25M, /* add option 25MHz, which is common */
        SDMMC_FREQ_DEFAULT
    ...
    /* select a freq from freq_values that is close to max freq of card */
    ...
    SDMMC_LOGD(TAG, "%s: using %d kHz bus frequency", __func__, selected_freq);
...
{
    bool print_scr = card->is_mmc ? false : true; /* emmc card do not have scr to read */
    bool print_csd = true;

    const char *type;
    fprintf(stream, "Probe Card Success !!!\n");
    fprintf(stream, "======================\n");
    ...
    fprintf(stream, "Manufactory ID: 0x%x\n", card->cid.mfg_id);
    fprintf(stream, "OEM ID: 0x%x\n", card->cid.oem_id);
    fprintf(stream, "Serial NO.: 0x%x\n", card->cid.serial);
    fprintf(stream, "Revision: 0x%x\n", card->cid.revision);
    ...
    if (print_csd)
    {
        fprintf(stream, "CSD: ver=%d, class=%d, max speed=%d, sector_size=%d, capacity=%d, read_bl_len=%d, \n",
                card->csd.csd_ver,
