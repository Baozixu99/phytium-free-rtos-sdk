...
sdmmc_err_t sdmmc_init_sd_scr(sdmmc_card_t *card)
{
    ...
    if ((card->scr.bus_width & SCR_SD_BUS_WIDTHS_4BIT) && (card->host.flags & SDMMC_HOST_FLAG_4BIT))
    {
...
sdmmc_err_t sdmmc_init_sd_bus_width(sdmmc_card_t *card)
{
    ...
    if (err != SDMMC_OK)
    {
        SDMMC_LOGE(TAG, "set_bus_width %d failed (0x%x)", width, err);
        return err;
    }
    return SDMMC_OK;
}
                                       ...
                                       sdmmc_switch_func_rsp_t *resp)
{
    ...
    {
        SDMMC_LOGE(TAG, "%s: got an invalid version of SWITCH_FUNC response: 0x%02x",
                   __func__, resp_ver);
...
sdmmc_err_t sdmmc_enable_hs_mode(sdmmc_card_t *card)
{
    ...
    sdmmc_switch_func_rsp_t *response = (sdmmc_switch_func_rsp_t *)
        sdmmc_port_align_malloc(sizeof(*response), TRUE);
    if (response == NULL)
    {
...
out:
    sdmmc_port_align_free(response);
    return err;
}
...
sdmmc_err_t sdmmc_decode_scr(uint32_t *raw_scr, sdmmc_scr_t *out_scr)
{
    ...
    SDMMC_LOGD(TAG, "sd spec: 0x%x, bus width: %d", out_scr->sd_spec, out_scr->bus_width);
    return SDMMC_OK;
}

sdmmc_err_t sdmmc_init_switch_hs(sdmmc_card_t *card)
{
    return sdmmc_send_cmd_switch(card);
}