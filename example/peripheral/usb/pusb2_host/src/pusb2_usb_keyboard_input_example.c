...
enum
{
    PUSB2_USB_KEYBOARD_SUCCESS = 0,
    PUSB2_USB_KEYBOARD_FAIL    = 1,
};
...
static void PUSB2UsbKeyboardTask(void *args)
{
    int ret;
    u8 id = (u8)usb_id;
...
task_exit:
    ret = (ret == 0) ? PUSB2_USB_KEYBOARD_SUCCESS : PUSB2_USB_KEYBOARD_FAIL;
    xQueueSend(xQueue, &ret, 0);
    vTaskDelete(NULL);    
}

BaseType_t FFreeRTOSRunPUSB2Keyboard(u32 id, const char *devname)
{
    BaseType_t ret = pdPASS;
    int task_res = PUSB2_USB_KEYBOARD_SUCCESS;
    ...
    ret = xTaskCreate((TaskFunction_t)PUSB2UsbKeyboardTask,
                      (const char *)"PUSB2UsbKeyboardTask",
                      (uint16_t)4096,
                      (void *)devname,
...
exit:
    ...
    if (task_res != PUSB2_USB_KEYBOARD_SUCCESS)
    {
        printf("%s@%d: PUSB2 usb keyboard example [failure], task_res = %d\r\n", __func__, __LINE__, task_res);
        return pdFAIL;
    }
    else
    {
        printf("%s@%d: PUSB2 usb keyboard example [success].\r\n", __func__, __LINE__);
        return pdTRUE;
    }
}