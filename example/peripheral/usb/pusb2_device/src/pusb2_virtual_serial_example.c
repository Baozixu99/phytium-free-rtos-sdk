 ...
 *
 *
 * FilePath: pusb2_virtual_serial_example.c
 * Date: 2022-02-10 14:53:42
 * LastEditTime: 2022-02-17 17:46:03
 * Description:  This file is for pusb2 cherryusb virtual serial example function implmentation
 *
 * Modify History:
...
static USB_NOCACHE_RAM_SECTION USB_MEM_ALIGNX uint8_t read_buffer[2048]; /* 2048 is only for test speed , please use CDC_MAX_MPS for common*/
...
static int cdc_acm_init(uint8_t busid, uint32_t reg_base)
{
    ...
    return usbd_initialize(busid, reg_base, usbd_event_handler);
}
...
{
    int ret = cdc_acm_init(id, usb_dc_get_register_base(id));

    if (ret == 0)
    {
        printf("%s@%d: PUSB2 virtual serial example [success] !!! \r\n", __func__, __LINE__);
    }
    else
    {
        printf("%s@%d: PUSB2 virtual serial example [failure] !!!, ret = %d \r\n", __func__, __LINE__, ret);
        return -1;
    }
