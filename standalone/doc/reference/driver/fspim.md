...
- FSpim *instance_p FSPIM驱动控制数据
- u32 option FSPIM操作标识数
- u32 value FSPIM用户自定参数
...
#### FSpimRegisterInterruptHandler

- SPIM中断事件注册函数

```c
void FSpimRegisterInterruptHandler(FSpim *instance_p, u32 evt, FSpimEvtHandler handler, void *param)
```
...
- 依据控制器config中的slave device id，控制相应cs上下拉
- 配合FSpimSetOption一起使用，可以做到控制任一cs上下拉
