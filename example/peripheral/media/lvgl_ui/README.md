...
LVGL (Light and Versatile Graphics Library) 是最流行的免费开源嵌入式图形库，可为任何 MCU、MPU 和显示类型创建漂亮的 UI，本例程展示如何利用LVGL与DP驱动，将串口调试信息显示到UI界面中。
...
当串口输入上述命令后，可在外接显示器界面中显示shell相关信息以及命令输入，此时无需再串口调试助手中输入命令，可直接通过显示器UI界面输入。

![lvgl_shell](fig/lvgl_shell.jpg)
